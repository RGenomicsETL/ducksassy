...
An outer SQL `LIMIT` limits rows returned by a query. The current scalar
functions first compute all matches for one text value as a LIST, then DuckDB
expands that LIST into rows. `LIMIT` may avoid work on later input rows, but it
cannot stop the native search or bound its memory within one value. A real
early-stop path would need a matcher that yields hits incrementally and a
stateful DuckDB table function that emits them in batches.

...
- `max_text_bytes` rejects a sequence **before** search when it exceeds 1 MiB
  by default. For a known long record, raise it in the call: the 4.6 Mb
  benchmark uses `max_text_bytes := 8388608`. This permits the search; it does
  not reserve or guarantee 8 MiB of memory.

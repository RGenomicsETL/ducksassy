...
DuckHTS streams *records*, but Sassy receives each record’s whole sequence and
can allocate RAM for search state and matches. DuckDB’s `memory_limit` does not
fully count that RAM or move it to disk. A long FASTA record is still one large
search, and parallel searches can use more RAM at once.

- `max_text_bytes` checks the sequence length **before** Sassy runs. It makes an
  unexpectedly long record an error; it is not a RAM limit. The 1 MiB default
  is a policy choice, not a measured safe threshold, and rejects any record
  over 1 MiB, including human nuclear chromosomes. The 4.6 Mb bacterial
  benchmark sets `max_text_bytes := 8388608`, still too small for those
  chromosomes. Raising the cap permits a larger search but gives no memory
  guarantee.
...
These limits raise errors rather than silently dropping hits. Splitting a long
reference into regions can reduce memory per search, but the caller must restore
reference coordinates and reconcile hits in overlapping regions; Ducksassy does

...
comparisons are between Ducksassy’s `scalar` and AVX2 backends, and between
Ducksassy builds using Sassy 0.2.1 and 0.2.6. The upstream Sassy CLI is used to
...
<details>
<summary>
<b>Memory and result limits</b>: cause, effect and options
</summary>

DuckHTS streams *records*, but Sassy searches one sequence against each pattern
and can hold hits and scratch space before DuckDB receives the result. A long
FASTA record is therefore still one large search. Separate DuckDB workers have
separate native searchers, so parallel searches can increase peak native memory.

- `max_text_bytes` is an opt-in guard against an unexpectedly long record
  entering native search. It rejects a sequence **before** search when it
  exceeds 1 MiB by default. That default is a policy choice, not a measured
  safe-memory threshold, and whole chromosomes exceed it. For a known long
  record, raise it in the call: the 4.6 Mb benchmark uses
  `max_text_bytes := 8388608`. This permits the search; it does not reserve or
  guarantee 8 MiB of memory.
- `max_hits` limits the hits returned for one sequence and its pattern panel.
  It defaults to 10,000 for value, FASTQ and table searches, or 1,000,000 for
  FASTA and CRISPR searches. The check runs **after each pattern search**, so a
  dense match set can consume native memory before it raises an error. Narrow
  the search or raise the cap only if the larger result is useful.
- DuckDB LIST output has a separate limit of 1,048,576 hits **across a chunk**.
  If that limit is reached, search a smaller input or use `sassy_count` or
  `sassy_contains` when individual hits are unnecessary.

These limits raise errors rather than silently dropping hits. Native search
allocations do not spill to disk and are not fully controlled by DuckDB’s
`memory_limit`. Splitting a long reference into regions can reduce per-search
memory, but the caller must restore reference coordinates and reconcile hits in
overlapping regions; Ducksassy does not do that automatically.

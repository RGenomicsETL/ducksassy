---
title: "Result buffer reuse and successful-call allocations"
output:
  github_document:
    html_preview: false
...
before <- read.csv(file.path(directory, "buffer_reuse_before", "timings.csv"))
after <- read.csv(file.path(directory, "buffer_reuse_after", "timings.csv"))
comparison <- merge(
  aggregate(elapsed_seconds ~ workload + mode, before, median),
  aggregate(elapsed_seconds ~ workload + mode, after, median),
  by = c("workload", "mode"), suffixes = c("_before", "_after")
)
comparison$reduction <- 100 * (1 - comparison$elapsed_seconds_after /
                                   comparison$elapsed_seconds_before)
checks <- read.csv(file.path(directory, "buffer_reuse_after", "checks.csv"))
```
...
```{r comparison}
knitr::kable(comparison, digits = c(0, 0, 3, 3, 1), row.names = FALSE,
